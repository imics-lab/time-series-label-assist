...
class TimeSeriesNP():
    ...
    def timeSlice(self, df):
        ...
        for i in range(0, len(df) - self.time_steps, self.step):
            ...
            # Check if all values in 'df_lbl' are 'Undefined'
            if (df_lbl.values == 'Undefined').all():
                pass
            else:
              # throw out undefined if all are not labeled as undefined
    ...
    def filter_segments(self, exclude_labels=['Undefined']):
        valid_indices = [i for i, label in enumerate(self.y) if label not in exclude_labels]
        self.x = self.x[valid_indices]
        self.y = self.y[valid_indices]
        self.sub = self.sub[valid_indices]
        self.time = self.time[valid_indices]

    def setArrays(self, df, labels, encode = True, one_hot_encode= True, filter=True):
        self.x, self.y, self.sub, self.time = self.timeSlice(df)

        if filter == True:
            # remove segments with unwanted labels
            self.filter_segments(['Other'])
